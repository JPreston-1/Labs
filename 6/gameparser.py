...
def filter_words(words, skip_words):
    ...
    """
    remove_words = [item for item in words if item not in skip_words]
    return remove_words
...
def normalise_input(user_input):
    ...
    #remove punctuation
    normalise_text = "".join((char for char in user_input.lower() if char not in string.punctuation))
    #split words into list format
    normalise_text = ' '.join(i for i in normalise_text.split() if i not in skip_words) 
    #print result
    return normalise_text.split()

    #normalised_text = "".join((char for char in user_input if char not in string.punctuation))
    #normalised_text_spaces = normalised_text.strip().lower()
    #return normalised_text_spaces
